...
router.get('/agg_sac_coverage', async (_req: Request, res: Response) => {
  try {
    const result = await pool.query(
      'SELECT year, iu_requiring_treatment, iu_treated, iu_effective_coverage, iu_requiring_treatment_pct, nat_sac_cov_pct, prog_sac_cov_pct FROM aggregates.agg_sac_coverage ORDER BY year'
    );
